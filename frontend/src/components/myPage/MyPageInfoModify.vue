  ...
  <div class="centerLine" @keydown.enter="formCheck">
    ...
    <div class="mainBox">
      ...
      <div>
        ...
        {{ store.nowLoginUser.userGender }}
      </div>
      <div>
        <label for="userBirthday" class="form-label">생년월일</label>
        {{ store.nowLoginUser.userBirthday }}
      </div>
    </div>

    <div>
      <button @click="formCheck" class="btn btn-primary">정보수정</button>
...
onMounted(() => {
  store.searchLoginUser();
});

const formData = ref({
  userPassword: "",
  userNickname: store.nowLoginUser.userNickname,
  userPhoneNumber: store.nowLoginUser.userPhoneNumber,
  userEmail: store.nowLoginUser.userEmail,
});
